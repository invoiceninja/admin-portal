...
class GroupRepository {
  const GroupRepository({this.webClient = const WebClient()});
  ...
  Future<GroupEntity> loadItem(
    ...
    final dynamic response = await webClient.get(
      '${credentials.url}/group_settings/$entityId',
      credentials.token,
    );

    final GroupItemResponse groupResponse = serializers.deserializeWith(
      GroupItemResponse.serializer,
      response,
    )!;
  ...
  Future<BuiltList<GroupEntity>> loadList(Credentials credentials) async {
    ...
    final GroupListResponse groupResponse = serializers.deserializeWith(
  ...
  Future<GroupEntity> saveData(
    ...
    if (group.isNew) {
      response = await webClient.post(
        credentials.url + '/group_settings',
        credentials.token,
