...
class ScheduleRepository {
  const ScheduleRepository({this.webClient = const WebClient()});
  ...
  Future<ScheduleEntity> loadItem(
    ...
    final dynamic response = await webClient.get(
      '${credentials.url}/task_schedulers/$entityId',
      credentials.token,
    );

    final ScheduleItemResponse scheduleResponse = serializers.deserializeWith(
      ScheduleItemResponse.serializer,
      response,
    )!;
  ...
  Future<BuiltList<ScheduleEntity>> loadList(Credentials credentials) async {
    ...
    final ScheduleListResponse scheduleResponse = serializers.deserializeWith(
  ...
  Future<ScheduleEntity> saveData(
    ...
    if (schedule.isNew) {
      response = await webClient.post(
        credentials.url + '/task_schedulers',
        credentials.token,
