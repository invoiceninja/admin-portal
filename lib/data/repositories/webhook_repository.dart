...
class WebhookRepository {
  const WebhookRepository({this.webClient = const WebClient()});
  ...
  Future<WebhookEntity> loadItem(
    ...
    final dynamic response = await webClient.get(
      '${credentials.url}/webhooks/$entityId',
      credentials.token,
    );

    final WebhookItemResponse webhookResponse = serializers.deserializeWith(
      WebhookItemResponse.serializer,
      response,
    )!;
  ...
  Future<BuiltList<WebhookEntity>> loadList(Credentials credentials) async {
    ...
    final WebhookListResponse webhookResponse = serializers.deserializeWith(
      WebhookListResponse.serializer,
      response,
    )!;
