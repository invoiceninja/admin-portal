...
class WidgetData {
  ...
  WidgetData.fromState(AppState state, AppLocalization? localization)
    : url = formatApiUrl(state.authState.url),
      ...
      companies = {
        for (var userCompany in state.userCompanyStates.where(
          (state) => state.company.hasName,
        ))
      ...
      dashboardFields = Map.fromIterable(
        <String>[
...
class WidgetCompany {
  ...
  WidgetCompany.fromUserCompany({
    required UserCompanyState userCompanyState,
    StaticState? staticState,
       ...
       firstMonthOfYear = parseInt(
         userCompanyState.userCompany.company.firstMonthOfYear,
       ),
...
class WidgetCurrency {
  WidgetCurrency({this.id, this.name, this.code, this.exchangeRate});
