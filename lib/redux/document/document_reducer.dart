...
ListUIState _viewDocumentList(
  ...
  ViewDocumentList action,
) {
  return documentListState.rebuild(
    (b) => b
      ..selectedIds = null
      ..filter = null
      ..filterClearedAt = DateTime.now().millisecondsSinceEpoch,
...
ListUIState _filterDocumentsByCustom1(
  ListUIState documentListState,
  FilterDocumentsByCustom1 action,
) {
...
ListUIState _filterDocumentsByCustom2(
  ListUIState documentListState,
  FilterDocumentsByCustom2 action,
) {
...
ListUIState _filterDocumentsByStatus(
  ListUIState documentListState,
  FilterDocumentsByStatus action,
) {
...
ListUIState _startListMultiselect(
  ListUIState documentListState,
  StartDocumentMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState documentListState,
  AddToDocumentMultiselect action,
) {
  return documentListState.rebuild(
...
ListUIState _removeFromListMultiselect(
  ListUIState documentListState,
  RemoveFromDocumentMultiselect action,
) {
  return documentListState.rebuild(
...
ListUIState _clearListMultiselect(
  ListUIState documentListState,
  ClearDocumentMultiselect action,
) {
...
DocumentState _deleteDocumentSuccess(
  DocumentState documentState,
  DeleteDocumentSuccess action,
) {
  return documentState.rebuild(
    (b) => b
...
DocumentState _addDocument(
  DocumentState documentState,
  AddDocumentSuccess action,
) {
  final state = documentState.rebuild(
    (b) => b
      ..map.addAll(
        Map.fromIterable(
          action.documents,
...
DocumentState _updateDocument(
  DocumentState documentState,
  SaveDocumentSuccess action,
) {
  return documentState.rebuild(
    (b) => b..map[action.document.id] = action.document,
  );
}

DocumentState _setLoadedDocument(
  DocumentState documentState,
  LoadDocumentSuccess action,
) {
  return documentState.rebuild(
    (b) => b..map[action.document.id] = action.document,
  );
...
DocumentState _setLoadedCompany(
  ...
  final state = documentState.rebuild(
    (b) => b
      ..map.addAll(
        Map<String, DocumentEntity>.fromIterable(
          documents,
          key: (dynamic item) => item.id,
          value: (dynamic item) => item,
        ),
