...
EntityUIState companyGatewayUIReducer(
  CompanyGatewayUIState state,
  dynamic action,
) {
  return state.rebuild(
    (b) => b
      ..listUIState.replace(
        companyGatewayListReducer(state.listUIState, action),
      )
...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.companyGateway
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _filterCompanyGatewaysByCustom1(
  ...
  if (companyGatewayListState.custom1Filters.contains(action.value)) {
    return companyGatewayListState.rebuild(
...
ListUIState _filterCompanyGatewaysByState(
  ListUIState companyGatewayListState,
  FilterCompanyGatewaysByState action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartCompanyGatewayMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToCompanyGatewayMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromCompanyGatewayMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearCompanyGatewayMultiselect action,
) {
...
CompanyGatewayState _addCompanyGateway(
  CompanyGatewayState companyGatewayState,
  AddCompanyGatewaySuccess action,
) {
  return companyGatewayState.rebuild(
    (b) => b
      ..map[action.companyGateway.id] = action.companyGateway.rebuild(
...
CompanyGatewayState _updateCompanyGateway(
  CompanyGatewayState companyGatewayState,
  SaveCompanyGatewaySuccess action,
) {
  return companyGatewayState.rebuild(
    (b) => b
      ..map[action.companyGateway.id] = action.companyGateway.rebuild(
...
CompanyGatewayState _setLoadedCompanyGateway(
  CompanyGatewayState companyGatewayState,
  LoadCompanyGatewaySuccess action,
) {
  return companyGatewayState.rebuild(
    (b) => b
      ..map[action.companyGateway.id] = action.companyGateway.rebuild(
...
CompanyGatewayState _setLoadedCompany(
  ...
  final state = companyGatewayState.rebuild(
    (b) => b
      ..map.addAll(
        Map.fromIterable(
