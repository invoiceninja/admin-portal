...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.bankAccount
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _viewBankAccountList(
  ...
  ViewBankAccountList action,
) {
  return bankAccountListState.rebuild(
    (b) => b
      ..selectedIds = null
      ..filter = null
      ..filterClearedAt = DateTime.now().millisecondsSinceEpoch,
...
ListUIState _filterBankAccountsByCustom1(
  ListUIState bankAccountListState,
  FilterBankAccountsByCustom1 action,
) {
...
ListUIState _filterBankAccountsByCustom2(
  ListUIState bankAccountListState,
  FilterBankAccountsByCustom2 action,
) {
...
ListUIState _filterBankAccountsByState(
  ListUIState bankAccountListState,
  FilterBankAccountsByState action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartBankAccountMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToBankAccountMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromBankAccountMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearBankAccountMultiselect action,
) {
...
BankAccountState _archiveBankAccountSuccess(
  ...
  ArchiveBankAccountsSuccess action,
) {
...
BankAccountState _deleteBankAccountSuccess(
  BankAccountState bankAccountState,
  DeleteBankAccountsSuccess action,
) {
  return bankAccountState.rebuild((b) {
    for (final bankAccount in action.bankAccounts) {
      b.map[bankAccount.id] = bankAccount;
...
BankAccountState _restoreBankAccountSuccess(
  BankAccountState bankAccountState,
  RestoreBankAccountsSuccess action,
) {
  return bankAccountState.rebuild((b) {
    for (final bankAccount in action.bankAccounts) {
      b.map[bankAccount.id] = bankAccount;
...
BankAccountState _addBankAccount(
  BankAccountState bankAccountState,
  AddBankAccountSuccess action,
) {
  return bankAccountState.rebuild(
    (b) => b
...
BankAccountState _updateBankAccount(
  BankAccountState bankAccountState,
  SaveBankAccountSuccess action,
) {
  return bankAccountState.rebuild(
    (b) => b..map[action.bankAccount.id] = action.bankAccount,
  );
}

BankAccountState _setLoadedBankAccount(
  BankAccountState bankAccountState,
  LoadBankAccountSuccess action,
) {
  return bankAccountState.rebuild(
    (b) => b..map[action.bankAccount.id] = action.bankAccount,
  );
