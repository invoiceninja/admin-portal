...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.transaction
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _viewTransactionList(
  ...
  ViewTransactionList action,
) {
  return transactionListState.rebuild(
    (b) => b
      ..selectedIds = null
      ..filter = null
      ..filterClearedAt = DateTime.now().millisecondsSinceEpoch,
...
ListUIState _filterTransactionsByCustom1(
  ListUIState transactionListState,
  FilterTransactionsByCustom1 action,
) {
...
ListUIState _filterTransactionsByCustom2(
  ListUIState transactionListState,
  FilterTransactionsByCustom2 action,
) {
...
ListUIState _filterTransactionsByState(
  ListUIState transactionListState,
  FilterTransactionsByState action,
) {
...
ListUIState _filterTransactionsByStatus(
  ListUIState transactionListState,
  FilterTransactionsByStatus action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartTransactionMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToTransactionMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromTransactionMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearTransactionMultiselect action,
) {
...
TransactionState _archiveTransactionSuccess(
  ...
  ArchiveTransactionsSuccess action,
) {
...
TransactionState _deleteTransactionSuccess(
  TransactionState transactionState,
  DeleteTransactionsSuccess action,
) {
  return transactionState.rebuild((b) {
    for (final transaction in action.transactions) {
      b.map[transaction.id] = transaction;
...
TransactionState _restoreTransactionSuccess(
  TransactionState transactionState,
  RestoreTransactionsSuccess action,
) {
  return transactionState.rebuild((b) {
    for (final transaction in action.transactions) {
      b.map[transaction.id] = transaction;
...
TransactionState _addTransaction(
  TransactionState transactionState,
  AddTransactionSuccess action,
) {
  return transactionState.rebuild(
    (b) => b
...
TransactionState _updateTransaction(
  TransactionState transactionState,
  SaveTransactionSuccess action,
) {
  return transactionState.rebuild(
    (b) => b..map[action.transaction.id] = action.transaction,
  );
}

TransactionState _convertTransactionToPayment(
  TransactionState transactionState,
...
TransactionState _convertTransactionToExpense(
  TransactionState transactionState,
...
TransactionState _linkTransactionToPayment(
  TransactionState transactionState,
  LinkTransactionToPaymentSuccess action,
) {
  return transactionState.rebuild(
    (b) => b..map[action.transaction.id] = action.transaction,
  );
...
TransactionState _linkTransactionToExpense(
  TransactionState transactionState,
  LinkTransactionToExpenseSuccess action,
) {
  return transactionState.rebuild(
    (b) => b..map[action.transaction.id] = action.transaction,
  );
...
TransactionState _convertTransactions(
  TransactionState transactionState,
  ConvertTransactionsSuccess action,
) {
  return transactionState.loadTransactions(action.transactions);
}
