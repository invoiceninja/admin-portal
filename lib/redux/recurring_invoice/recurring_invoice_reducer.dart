...
EntityUIState recurringInvoiceUIReducer(
  RecurringInvoiceUIState state,
  dynamic action,
) {
  return state.rebuild(
    (b) => b
      ..listUIState.replace(
        recurringInvoiceListReducer(state.listUIState, action),
      )
      ..editing.replace(editingReducer(state.editing, action)!)
      ..editingItemIndex = editingItemIndexReducer(
...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.recurringInvoice
        ? action.entityId
        : selectedId,
  ),
...
final editingReducer = combineReducers<InvoiceEntity?>([
  ...
  TypedReducer<InvoiceEntity?, UpdateRecurringInvoiceClient>((
    ...
    return recurringInvoice!.rebuild(
      (b) => b
        ..isChanged = true
        ..clientId = client?.id ?? ''
        ..invitations.replace(
          (client?.emailContacts ?? <ClientContactEntity>[])
  ...
  TypedReducer<InvoiceEntity?, RemoveRecurringInvoiceContact>((
    ...
    return recurringInvoice!.rebuild(
      (b) => b..invitations.remove(action.invitation),
    );
  }),
]);
...
InvoiceEntity _addRecurringInvoiceItem(
  ...
  AddRecurringInvoiceItem action,
) {
...
InvoiceEntity _addRecurringInvoiceItems(
  InvoiceEntity? recurringInvoice,
  AddRecurringInvoiceItems action,
) {
...
InvoiceEntity? _removeRecurringInvoiceItem(
  InvoiceEntity? recurringInvoice,
  DeleteRecurringInvoiceItem action,
) {
...
InvoiceEntity? _updateRecurringInvoiceItem(
  InvoiceEntity? recurringInvoice,
  UpdateRecurringInvoiceItem action,
) {
  if (recurringInvoice!.lineItems.length <= action.index) {
    return recurringInvoice;
  }
  return recurringInvoice.rebuild(
    (b) => b..lineItems[action.index] = action.item,
  );
}
...
ListUIState _filterRecurringInvoicesByCustom1(
  ...
  if (recurringInvoiceListState.custom1Filters.contains(action.value)) {
    return recurringInvoiceListState.rebuild(
...
ListUIState _filterRecurringInvoices(
  ListUIState recurringInvoiceListState,
  FilterRecurringInvoices action,
) {
  return recurringInvoiceListState.rebuild(
    (b) => b
...
RecurringInvoiceState _emailRecurringInvoiceSuccess(
  ...
  return recurringInvoiceState.rebuild(
    (b) => b..map[action.invoice.id] = action.invoice,
  );
}
...
RecurringInvoiceState _addRecurringInvoice(
  ...
  return recurringInvoiceState.rebuild(
    (b) => b
      ..map[action.recurringInvoice.id] = action.recurringInvoice.rebuild(
...
RecurringInvoiceState _updateRecurringInvoice(
  ...
  return recurringInvoiceState.rebuild(
    (b) => b
      ..map[action.recurringInvoice.id] = recurringInvoice!.rebuild(
        (b) => b..loadedAt = DateTime.now().millisecondsSinceEpoch,
      ),
  );
}
