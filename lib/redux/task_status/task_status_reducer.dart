...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.taskStatus
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _viewTaskStatusList(
  ...
  ViewTaskStatusList action,
) {
  return taskStatusListState.rebuild(
    (b) => b
      ..selectedIds = null
      ..filter = null
      ..filterClearedAt = DateTime.now().millisecondsSinceEpoch,
...
ListUIState _filterTaskStatusesByCustom1(
  ListUIState taskStatusListState,
  FilterTaskStatusesByCustom1 action,
) {
...
ListUIState _filterTaskStatusesByCustom2(
  ListUIState taskStatusListState,
  FilterTaskStatusesByCustom2 action,
) {
...
ListUIState _filterTaskStatusesByState(
  ListUIState taskStatusListState,
  FilterTaskStatusesByState action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartTaskStatusMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToTaskStatusMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromTaskStatusMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearTaskStatusMultiselect action,
) {
...
TaskStatusState _sortTaskStatusSuccess(
  ...
  SortTasksSuccess action,
) {
  return taskStatusState.rebuild((b) {
    for (final statusId in action.statusIds!) {
      b.map[statusId] = taskStatusState.map[statusId]!.rebuild(
        (b) => b..statusOrder = action.statusIds!.indexOf(statusId),
      );
    }
  });
}
...
TaskStatusState _archiveTaskStatusSuccess(
  TaskStatusState taskStatusState,
  ArchiveTaskStatusesSuccess action,
) {
  return taskStatusState.rebuild((b) {
...
TaskStatusState _deleteTaskStatusSuccess(
  TaskStatusState taskStatusState,
  DeleteTaskStatusesSuccess action,
) {
  return taskStatusState.rebuild((b) {
    for (final taskStatus in action.taskStatuses) {
      b.map[taskStatus.id] = taskStatus;
...
TaskStatusState _restoreTaskStatusSuccess(
  TaskStatusState taskStatusState,
  RestoreTaskStatusesSuccess action,
) {
  return taskStatusState.rebuild((b) {
    for (final taskStatus in action.taskStatuses) {
      b.map[taskStatus.id] = taskStatus;
...
TaskStatusState _addTaskStatus(
  TaskStatusState taskStatusState,
  AddTaskStatusSuccess action,
) {
  return taskStatusState.rebuild(
    (b) => b
...
TaskStatusState _updateTaskStatus(
  TaskStatusState taskStatusState,
  SaveTaskStatusSuccess action,
) {
  return taskStatusState.rebuild(
    (b) => b..map[action.taskStatus.id] = action.taskStatus,
  );
}

TaskStatusState _setLoadedTaskStatus(
  TaskStatusState taskStatusState,
  LoadTaskStatusSuccess action,
) {
  return taskStatusState.rebuild(
    (b) => b..map[action.taskStatus.id] = action.taskStatus,
  );
