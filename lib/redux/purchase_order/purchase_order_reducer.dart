...
EntityUIState purchaseOrderUIReducer(
  ...
  return state.rebuild(
    (b) => b
      ...
      ..historyActivityId = historyActivityIdReducer(
...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.purchaseOrder
        ? action.entityId
        : selectedId,
  ),
  ...
  TypedReducer<InvoiceEntity?, UpdatePurchaseOrderVendor>((
    ...
    return purchaseOrder!.rebuild(
      (b) => b
        ...
        ..vendorId = vendor?.id ?? ''
        ..invitations.replace(
          (vendor?.emailContacts ?? <VendorContactEntity>[])
...
InvoiceEntity _addPurchaseOrderItem(
  ...
  if (action.index == null) {
    ...
    return purchaseOrder!.rebuild(
      (b) => b..lineItems.insert(action.index!, item),
    );
  }
}
...
InvoiceEntity? _updatePurchaseOrderItem(
  ...
  return purchaseOrder.rebuild(
    (b) => b..lineItems[action.index] = action.purchaseOrderItem,
  );
}
...
ListUIState _viewPurchaseOrderList(
  ListUIState purchaseOrderListState,
  ViewPurchaseOrderList action,
) {
  return purchaseOrderListState.rebuild(
    (b) => b
      ..selectedIds = null
      ..filter = null
      ..filterClearedAt = DateTime.now().millisecondsSinceEpoch,
...
ListUIState _filterPurchaseOrdersByCustom1(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByCustom1 action,
) {
...
ListUIState _filterPurchaseOrdersByCustom2(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByCustom2 action,
) {
...
ListUIState _filterPurchaseOrdersByCustom3(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByCustom3 action,
) {
  if (purchaseOrderListState.custom3Filters.contains(action.value)) {
    ...
    return purchaseOrderListState.rebuild(
      (b) => b..custom3Filters.add(action.value),
    );
  }
}
...
ListUIState _filterPurchaseOrdersByCustom4(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByCustom4 action,
) {
  if (purchaseOrderListState.custom4Filters.contains(action.value)) {
    ...
    return purchaseOrderListState.rebuild(
      (b) => b..custom4Filters.add(action.value),
    );
  }
}
...
ListUIState _filterPurchaseOrdersByState(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByState action,
) {
...
ListUIState _filterPurchaseOrdersByStatus(
  ListUIState purchaseOrderListState,
  FilterPurchaseOrdersByStatus action,
) {
...
ListUIState _startListMultiselect(
  ListUIState purchaseOrderListState,
  StartPurchaseOrderMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState purchaseOrderListState,
  AddToPurchaseOrderMultiselect action,
) {
  return purchaseOrderListState.rebuild(
    (b) => b..selectedIds.add(action.entity!.id),
  );
}
...
ListUIState _clearListMultiselect(
  ListUIState purchaseOrderListState,
  ClearPurchaseOrderMultiselect action,
) {
...
PurchaseOrderState _acceptPurchaseOrderSuccess(
  PurchaseOrderState purchaseOrderState,
  AcceptPurchaseOrderSuccess action,
) {
  final purchaseOrderMap = Map<String, InvoiceEntity>.fromIterable(
    action.purchaseOrders,
    key: (dynamic item) => item.id,
...
PurchaseOrderState _cancelPurchaseOrderSuccess(
  PurchaseOrderState purchaseOrderState,
  CancelPurchaseOrderSuccess action,
) {
  final purchaseOrderMap = Map<String, InvoiceEntity>.fromIterable(
    action.purchaseOrders,
    key: (dynamic item) => item.id,
...
PurchaseOrderState _deletePurchaseOrderSuccess(
  PurchaseOrderState purchaseOrderState,
  DeletePurchaseOrdersSuccess action,
) {
  return purchaseOrderState.rebuild((b) {
    for (final purchaseOrder in action.purchaseOrders) {
      b.map[purchaseOrder.id] = purchaseOrder;
...
PurchaseOrderState _emailPurchaseOrderSuccess(
  PurchaseOrderState purchaseOrderState,
  EmailPurchaseOrderSuccess action,
) {
  return purchaseOrderState.rebuild(
    (b) => b..map[action.purchaseOrder.id] = action.purchaseOrder,
  );
}

PurchaseOrderState _approvePurchaseOrderSuccess(
  PurchaseOrderState purchaseOrderState,
  ApprovePurchaseOrderSuccess action,
) {
  final purchaseOrderMap = Map<String, InvoiceEntity>.fromIterable(
    action.purchaseOrders!,
    key: (dynamic item) => item.id,
...
PurchaseOrderState _addPurchaseOrder(
  PurchaseOrderState purchaseOrderState,
  AddPurchaseOrderSuccess action,
) {
  return purchaseOrderState.rebuild(
    (b) => b
      ..map[action.purchaseOrder.id] = action.purchaseOrder.rebuild(
