...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.paymentTerm
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _filterPaymentTermsByCustom1(
  ListUIState paymentTermListState,
  FilterPaymentTermsByCustom1 action,
) {
...
ListUIState _filterPaymentTermsByCustom2(
  ListUIState paymentTermListState,
  FilterPaymentTermsByCustom2 action,
) {
...
ListUIState _filterPaymentTermsByState(
  ListUIState paymentTermListState,
  FilterPaymentTermsByState action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartPaymentTermMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToPaymentTermMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromPaymentTermMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearPaymentTermMultiselect action,
) {
...
PaymentTermState _archivePaymentTermSuccess(
  ...
  ArchivePaymentTermsSuccess action,
) {
...
PaymentTermState _deletePaymentTermSuccess(
  PaymentTermState paymentTermState,
  DeletePaymentTermsSuccess action,
) {
  return paymentTermState.rebuild((b) {
    for (final paymentTerm in action.paymentTerms) {
      b.map[paymentTerm.id] = paymentTerm;
...
PaymentTermState _restorePaymentTermSuccess(
  PaymentTermState paymentTermState,
  RestorePaymentTermsSuccess action,
) {
  return paymentTermState.rebuild((b) {
    for (final paymentTerm in action.paymentTerms) {
      b.map[paymentTerm.id] = paymentTerm;
...
PaymentTermState _addPaymentTerm(
  PaymentTermState paymentTermState,
  AddPaymentTermSuccess action,
) {
  return paymentTermState.rebuild(
    (b) => b
...
PaymentTermState _updatePaymentTerm(
  PaymentTermState paymentTermState,
  SavePaymentTermSuccess action,
) {
  return paymentTermState.rebuild(
    (b) => b..map[action.paymentTerm.id] = action.paymentTerm,
  );
}

PaymentTermState _setLoadedPaymentTerm(
  PaymentTermState paymentTermState,
  LoadPaymentTermSuccess action,
) {
  return paymentTermState.rebuild(
    (b) => b..map[action.paymentTerm.id] = action.paymentTerm,
  );
...
PaymentTermState _setLoadedCompany(
  ...
  final state = paymentTermState.rebuild(
    (b) => b
      ..map.addAll(
        Map.fromIterable(
