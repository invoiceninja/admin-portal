...
DashboardUIState dashboardUIReducer(DashboardUIState state, dynamic action) {
  return state.rebuild(
    (b) => b
      ...
      ..selectedEntities.replace(
        selectedEntitiesReducer(state.selectedEntities, action),
      )
...
Reducer<BuiltMap<EntityType?, BuiltList<String>>> selectedEntitiesReducer =
    combineReducers([
      TypedReducer<
        BuiltMap<EntityType?, BuiltList<String>>,
        UpdateDashboardSelection
      >((state, action) {
        return state.rebuild(
          (b) => b
            ..[action.entityType] = BuiltList(action.entityIds ?? <String>[]),
        );
      }),
