...
Reducer<String?> selectedIdReducer = combineReducers([
  ...
  TypedReducer<String?, PreviewEntity>(
    (selectedId, action) => action.entityType == EntityType.paymentLink
        ? action.entityId
        : selectedId,
  ),
...
ListUIState _filterSubscriptionsByCustom1(
  ...
  FilterSubscriptionsByCustom1 action,
) {
  if (subscriptionListState.custom1Filters.contains(action.value)) {
    return subscriptionListState.rebuild(
...
ListUIState _filterSubscriptionsByCustom2(
  ListUIState subscriptionListState,
  FilterSubscriptionsByCustom2 action,
) {
...
ListUIState _filterSubscriptionsByState(
  ListUIState subscriptionListState,
  FilterSubscriptionsByState action,
) {
...
ListUIState _startListMultiselect(
  ...
  StartSubscriptionMultiselect action,
) {
...
ListUIState _addToListMultiselect(
  ListUIState productListState,
  AddToSubscriptionMultiselect action,
) {
...
ListUIState _removeFromListMultiselect(
  ListUIState productListState,
  RemoveFromSubscriptionMultiselect action,
) {
...
ListUIState _clearListMultiselect(
  ListUIState productListState,
  ClearSubscriptionMultiselect action,
) {
...
SubscriptionState _archiveSubscriptionSuccess(
  ...
  ArchiveSubscriptionsSuccess action,
) {
...
SubscriptionState _restoreSubscriptionSuccess(
  SubscriptionState subscriptionState,
  RestoreSubscriptionsSuccess action,
) {
  return subscriptionState.rebuild((b) {
    for (final subscription in action.subscriptions) {
      b.map[subscription.id] = subscription;
...
SubscriptionState _addSubscription(
  SubscriptionState subscriptionState,
  AddSubscriptionSuccess action,
) {
  return subscriptionState.rebuild(
    (b) => b
...
SubscriptionState _updateSubscription(
  SubscriptionState subscriptionState,
  SaveSubscriptionSuccess action,
) {
  return subscriptionState.rebuild(
    (b) => b..map[action.subscription.id] = action.subscription,
  );
}

SubscriptionState _setLoadedSubscription(
  SubscriptionState subscriptionState,
  LoadSubscriptionSuccess action,
) {
  return subscriptionState.rebuild(
    (b) => b..map[action.subscription.id] = action.subscription,
  );
...
SubscriptionState _setLoadedSubscriptions(
  SubscriptionState subscriptionState,
  LoadSubscriptionsSuccess action,
