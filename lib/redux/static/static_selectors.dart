...
var memoizedGroupList = memo1(
  (BuiltMap<String, GroupEntity> groupMap) => groupList(groupMap),
);
...
var memoizedLanguageList = memo1(
  (BuiltMap<String, LanguageEntity> languageMap) => languageList(languageMap),
);
...
var memoizedCurrencyList = memo1(
  (BuiltMap<String, CurrencyEntity> currencyMap) => currencyList(currencyMap),
);
...
var memoizedTimezoneList = memo1(
  (BuiltMap<String, TimezoneEntity> timezoneMap) => timezoneList(timezoneMap),
);
...
var memoizedIndustryList = memo1(
  (BuiltMap<String?, IndustryEntity?> industryMap) => industryList(industryMap),
);
...
var memoizedSizeList = memo1(
  (BuiltMap<String, SizeEntity> sizeMap) => sizeList(sizeMap),
);
...
List<String?> gatewayList(
  BuiltMap<String, GatewayEntity> gatewayMap,
  bool isHosted,
) {
  final list = gatewayMap.keys.where((gatewayId) {
    ...
    if (isHosted) {
      if ([kGatewayPayPalExpress, kGatewayPayPalREST].contains(gateway.id)) {
        return false;
      }
    } else {
      if ([kGatewayPayPalPlatform].contains(gateway.id)) {
        return false;
      }
    }
